...
const scrapeIkman = async (keyword) => {
  ...
  const searchUrl = `https://ikman.lk/en/ads/sri-lanka?query=${encodeURIComponent(
    keyword
  )}`;
  ...
  const listings = await page.evaluate(() => {
    ...
    items.forEach((item) => {
      ...
      const priceElement = item
        .closest("div")
        .querySelector(".price--3SnqI span");
      // Find location within the same parent
      const locationElement = item
        .closest("div")
        .querySelector(".description--2-ez3");
      // Find date within the same parent
      const dateElement = item
        .closest("div")
        .querySelector(".updated-time--1DbCk span");
